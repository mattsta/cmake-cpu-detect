...
CMAKE_HELPER = "x64FeatureDetect.cmake"
...
def writeCMakeHelper(featureMarchMap):
    with open(CMAKE_HELPER, "w") as x:
        ...
        x.write("include(CheckSymbolExists)\n")
...
""".format())  # empty format() because we already doubled all the {{}}

        # In cmake, use:
        # if (CLANG_FEATURE_DETECTED_AVX2) ...
        ...
        def printCMakeDetectionCapability(feature, marchs):
            x.write("""
message(STATUS "Checking for {}...")
CHECK_SYMBOL_EXISTS(__{}__ stdlib.h CLANG_FEATURE_DETECTED_{})
set(${{RESULT_PREFIX}}_{} CLANG_FEATURE_DETECTED_{} PARENT_SCOPE)
""".format(feature, feature, feature, feature, feature))

            # Add a catch-all "AVX512 support" query
            if feature == "AVX512F":
                x.write(
                    """set(${{RESULT_PREFIX}}_AVX512 CLANG_FEATURE_DETECTED_{} PARENT_SCOPE)
""".format(feature))
...
message(STATUS "Querying for {}...")
foreach(marchName IN LISTS CLANG_FEATURE_MAP_{})
if ("${{BUILDING_FLAGS}}" MATCHES "march=${{marchName}}")
set(${{RESULT_PREFIX}}_{} True PARENT_SCOPE)
""".format(feature, " ".join(marchs), feature, feature, feature))

            # Add a catch-all "AVX512 support" query
            if feature == "AVX512F":
                x.write("""set(${{RESULT_PREFIX}}_AVX512 True PARENT_SCOPE)
""".format(feature))
